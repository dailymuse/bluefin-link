...
test("rejects a table mock that is not an array", t => {
  ...
  return t.context.db
    .connect(sql => sql.selectSeries(8))
    .catch(e => {
      t.is(e.name, "Error");
...
test("includes the error name and message in the stack", t => {
  ...
  return db
    .connect(sql => sql.errorWithArguments(42, 21, 96))
    .catch(e => {
      ...
      t.regex(
        e.effect.stack,
        /^Error: mock errorWithArguments\(\) threw error/
      );
      t.deepEqual(e.effect.context.arguments, [42, 21, 96]);
      t.true(
        e.effect.context.source.endsWith("test/sql/errorWithArguments.sql")
      );
